...
for index, job_element in enumerate(data['catalogs'], start=1):
    ...
    sliderimageSRC=job_element['collage_image']   
    ...
    TotalImages= "images/" + image_filename + "," + "images/" + image_filename2
    ...
    # `full_details` contains the complete comma-separated size list. The
    # share text only has the first size followed by measurement details.
    readSizes = job_element.get("full_details", "")
    match = re.search(r"Sizes:\s*([\s\S]*?)(?:\nDispatch:|$)", readSizes)
    if match:
        sizes_str = match.group(1).replace("\n", ",")
    ...
    product_data = {
        ...
        "categories": 'Women > Western Wear Ladies > Top',
    ...
    for sizeValue in sizes:
        ...
        _, doc_ref = db.collection("products").add(product_data2)
